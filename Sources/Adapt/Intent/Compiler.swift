/*
 *  Adapt: An implicit programing language based on SWIFT
 *
...
public class Compiler {
    ...
    public func splitIntentIntoSections(source fileContent: String) -> [String : String] {
        ...
        guard let knobSection = extractSection("knobs") else {
            Adapt.fatalError("Could not extract knobs section from intent specification: '\(fileContent)'.")
        }
    ...
    */
    func separateKnobConstraintsFromRestOfIntentSpec(source fileContent: String) -> (String,String?) {
        ...
        switch knobSectionComponents.count {
            ...
            default:
                Adapt.fatalError("Malformed knob section: '\(knobSection)'.")
        }
...
print("MER!!?")
            self.costOrValue = { measureValuesArray in
                ...
                guard let result = try? expression.evaluate() as Double else {
                    Adapt.fatalError("Failed to evaluate objective function: '\(expression)', as a Double, in environment: '\(measureValuesDictionary)'.")
                }
                return result
            }
        }
        ...
        public func satisfiesKnobConstraints(knobSettings: KnobSettings) -> Bool {
            ...
            func evalBoolOp(_ op: String, l: Any, r: Any, _ function: (Bool,Bool) -> Bool) -> Bool {
                guard let leftBool = l as? Bool else {
                    Adapt.fatalError("Left operand for knob constraint \(l) \(op) \(r) is not boolean.")
                }
                guard let rightBool = r as? Bool else {
                    Adapt.fatalError("Right operand for knob constraint \(l) \(op) \(r) is not boolean.")
                }
            ...
            func equalTo(_ l: Any, _ r: Any) -> Bool {
                switch (l,r) {
                    ...
                    case let (la,ra) as ([Any],[Any]):
                        ...
                        return true
                    default:
                        Adapt.fatalError("Equality check not implemented for: \((type(of:l), type(of:r)))")
                }
            }  
            ...
            guard let result = try? expression.evaluate() as Bool else {
                Adapt.fatalError("Failed to evaluate knob constraints: '\(expression)', as a boolean, in environment: '\(knobSettings.settings)' (from KnobSettings with id \(knobSettings.kid)).")
            }
            return result
        }*/
    ...
    internal func compileTypedLiteral<T>(_ e: Expression) -> T? {
            if let t = compileLiteralOption(e) as? T {
                ...
                Adapt.fatalError("Mistyped literal: \(compileLiteralOption(e)).")
                return nil
            } 
    }
        ...
        if let le = e as? LiteralExpression {
            switch le.kind {
                ...
                case .staticString(let s, _):
                    return s
                default: 
                    Adapt.fatalError("Unsupported literal: \(le).")
            }
        }
        else {
            Adapt.fatalError("Expected literal in compileLiteral. Found: \(e).")
        }
    }

    internal func compileKnobLiteral(_ e: Expression) -> KnobValue {
        Log.debug("Compiling literal expression '\(e)'.")
        if let le = e as? LiteralExpression {
            switch le.kind {
                case .integer(let i, _):
    ...
    internal func compileRange(_ range: Expression) -> [KnobValue] {
        switch (range as! LiteralExpression).kind {
            case .array(let elements):
                return elements.map{ compileKnobLiteral($0) }
            default: 
                Adapt.fatalError("Could not compile knob range in compileRange: \(range).")
        }
    }
                ...
                case .array(let elements):
                    return elements.map{ 
                        if let s: String = compileTypedLiteral($0) {
                            ...
                            Adapt.fatalError("Expected training set element string in compileTrainingSet. Found: \($0).")
                        }
                    }
                default: 
                    Adapt.fatalError("Unsupported training set descriptor found in compileTrainingSet: \(commands.kind).")
            }
        }
        else {
            Adapt.fatalError("Could not compile training set descriptor found in compileTrainingSet: \(commandsExpr).")
        }
    }
